...
import test from '@/components/MDXImage/test';
...
[...posts]
      ...
      // posts.filter((post) =>
      //   category !== "All" ? post?.slug.split("/")[1] === category : post
      // )
        // ...posts,
        // {
        //   title:'MSW title' , 
  ...
  http.get('/api/test/:category' , ({request ,params})=>{
    const {category } = params ;
    let infiniteData;
    
    // console.log('/api/test/:category   ==' , category);
    
    const url = new URL(request.url)
    const cursor = parseInt(url.searchParams.get('cursor') as string) || 0
    // const postData = url.searchParams.get('postData') ;
    ...
    if(cursor > copytestData.length+3 ) {
      infiniteData=undefined
    }else {
      infiniteData = copytestData.filter( (p) => p.pageNum >= cursor  &&   p.pageNum <= cursor+2 
    )
